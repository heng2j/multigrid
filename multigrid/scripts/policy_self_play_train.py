...
def get_cli_args():
    ...
    parser.add_argument(
        ...
        choices=["markov_soccer", "connect_four", "MultiGrid-CompetativeRedBlueDoor-v3-DTDE-1v1"],
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    from multigrid.utils.training_utilis import model_config
    from ray.rllib.utils.from_config import NotProvided

    config = (
        PPOConfig()
        .environment(env="MultiGrid-CompetativeRedBlueDoor-v3-DTDE-1v1", env_config=my_config.env_config)
        ...
        .training(
            model=model_config(
                framework=args.framework,
                lstm=False,
                custom_model_config={"teams": my_config.env_config["teams"], "training_scheme": my_config.env_config["training_scheme"]},
            ),
            lr=NotProvided,
        )
        .multi_agent(
            ...
            # another "main").
            policy_mapping_fn=lambda agent_id, episode, worker, **kwargs: agent_id, # self_play_policy_mapping_fn, #lambda agent_id, episode, worker, **kwargs: agent_id,
