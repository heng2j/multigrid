...
SUBMISSION_CONFIG_FILE =  sorted(Path("submission").expanduser().glob('**/submission_config.json'), key=os.path.getmtime)[-1]
...
def test_evaluation():
    ...
    checkpoint_dirs = [checkpoint_dir.parent for checkpoint_dir in sorted(Path(search_dir).expanduser().glob('**/result.json'), key=os.path.getmtime)]

    checkpoint_paths = [sorted(Path(checkpoint_dir).expanduser().glob('**/*.is_checkpoint'), key=os.path.getmtime)[-1].parent for checkpoint_dir in checkpoint_dirs ]
